...
'Run croupier on guessing game with different parameters and store the output in files in a directory.'
...
def create_output(dirpath):
    ...
    for fname, label_args in file_label_args:
        with open(os.path.join(dirpath, fname + '_100.txt'), 'w') as fobj:
            ...
            args = ['python', CROUPIER_PATH, solution_args, judge_args] + label_args
            subprocess.check_call(args, stderr=DEVNULL, stdout=fobj)

def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('dir', help='path to directory where output files should be created')
